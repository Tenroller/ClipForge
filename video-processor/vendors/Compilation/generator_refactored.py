...
try:
    # Import from video processor root
    import sys
    from pathlib import Path
    video_processor_root = Path(__file__).resolve().parent.parent.parent
    if str(video_processor_root) not in sys.path:
        sys.path.insert(0, str(video_processor_root))
except ImportError:
    # Fallback font function if module is not available
    def get_font_fallback_list():
        return ["Arial", "Liberation Sans", "DejaVu Sans", "Helvetica"]
