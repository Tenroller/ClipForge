...
class InputValidator:
    ...
    def validate_compilation_request(self, request: CompilationRequest) -> ValidationResult:
        ...
        elif request.min_duration < self.min_compilation_duration:
            result.add_warning(f"Minimum duration very short: {request.min_duration}s")
        
        if request.max_duration <= 0:
            result.add_error(f"Maximum duration must be positive: {request.max_duration}")
        elif request.max_duration > self.max_compilation_duration:
            result.add_warning(f"Maximum duration very long: {request.max_duration}s")
