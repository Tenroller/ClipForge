...
from moviepy import ImageSequenceClip, CompositeVideoClip
...
class OptimizedSubtitleRenderer:
        ...
        for word_data in window_words:
            ...
            try:
                ...
                try:
                    ...
                    word_width = len(word_text) * (font.size * 0.6) if hasattr(font, 'size') else len(word_text) * 12
                    word_height = font.size if hasattr(font, 'size') else 20
    ...
    def _get_font_paths(self, font_family: str) -> List[Optional[str]]:
        """Get list of font paths to try."""
        ...
        font_names = [
            ...
            f"{font_family.replace('-', '').lower()}.ttf",
        ]
        ...
        # Try specific font files in common locations
        ...
        # Add system font name (let PIL handle it)
